...
import { Home, Swords, Radio, PlayCircle, BarChart3, User, LogOut, Sparkles, AlertTriangle, Heart, LayoutGrid, type LucideIcon } from 'lucide-react'
...
export function SidebarNav({ isAuthenticated, onLogout, onNavigate }: SidebarNavProps) {
  ...
  return (
    <nav className="mt-4 grid items-start gap-1.5 text-sm font-medium">
      ...
      {isAuthenticated && <><h3 className="px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-[0.28em] text-text-muted sm:text-[11px]">My Account</h3><NavItem href="/profile" label="My Profile" icon={User} onClick={onNavigate} /><button onClick={() => { onNavigate?.(); onLogout() }} className="relative flex w-full items-center gap-2.5 rounded-xl border border-transparent px-2.75 py-2.5 text-left text-[13px] font-semibold text-text-muted transition duration-200 ease-in-out hover:border-border hover:bg-surface-soft hover:text-text-primary hover:shadow-nav-active"><div className="nav-connector-main nav-connector-main-last" aria-hidden="true" /><LogOut className="relative z-10 h-6 w-6 shrink-0" /><span className="relative z-10 truncate">Logout</span></button></>}
    </nav>
  )
}
