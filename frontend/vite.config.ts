...
const pwaVersionPlugin = {
  ...
  closeBundle() {
    ...
    if (worker === versionedWorker) throw new Error('Service worker version placeholder was not found.')
    ...
    rollupOptions: {
      ...
      output: {
        ...
        manualChunks(id) {
          ...
          // Data & tables
          if (id.includes('@tanstack') || id.includes('tanstack')) return 'vendor-data'
          ...
          if (id.includes('react-day-picker') || id.includes('day-picker')) return 'vendor-date-picker'
          ...
          if (id.includes('ms')) return 'vendor-ms'
          
          // Services & backend communication
          if (id.includes('@supabase')) return 'vendor-supabase'
          if (id.includes('@logtail')) return 'vendor-logging'

          // Notifications & UI feedback
          if (id.includes('sonner')) return 'vendor-toast'
          if (id.includes('nprogress')) return 'vendor-progress'
          ...
          if (id.includes('class-variance')) return 'vendor-cva'

          // React utilities
          if (id.includes('react-is')) return 'vendor-react-is'
          if (id.includes('use-sync-external-store')) return 'vendor-sync-store'
          ...
          if (id.includes('hoist-non-react-statics')) return 'vendor-hoist'
          ...
          if (id.includes('fast-equals')) return 'vendor-fast-equals'
          if (id.includes('memoize-one')) return 'vendor-memoize'
          if (id.includes('resolve-pathname')) return 'vendor-resolve-pathname'
          if (id.includes('value-equal')) return 'vendor-value-equal'
          if (id.includes('shallowequal')) return 'vendor-shallowequal'

          // UI & cropping utilities
          if (id.includes('async-mutex')) return 'vendor-async-mutex'
          if (id.includes('react-easy-crop')) return 'vendor-easy-crop'
